                    ...
                    case "1️⃣":
                        var confirmationMsg = await messageReaction.message.channel.send({
                            embed: {
                                ...
                                "fields": [
                                    {
                                        "name": "The data that the Chrome extension will allow you to copy will contain **sensitive information** that can only be abused by people who know what they're doing.",
                                        "value": "Though it doesn't store your username or password, it can grant people access to your account. However, it is vital for the bot to use it so that it is able to see assignments in classes it doesn't have access to."
                                    },
                                    {
                                        "name": "WE WILL NOT USE THIS ACCESS WITH MALICIOUS INTENT. You can check the source code.",
                        ...
                        await confirmationMsg.react("❌");
                        awaitingConfirmation.delete(user.id);
                        awaitingConfirmation.set(messageReaction.message.author.id, {confirmationMessage: confirmationMsg.id, for: ConfirmationFor.COOKIE_FILE});
                        break;
                    case "2️⃣":
                        await messageReaction.message.channel.send("This method should be used as a fallback if the Chrome extension does not work. If you would like to continue, please upload the file named `Cookies` located at `%appdata%/Microsoft/Teams` to this DM.");
                        break;
                }
